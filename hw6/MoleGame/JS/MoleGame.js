...
window.onload = function() {
    ...
    document.getElementById('start_and_stop_button').onclick = function() {
        if (!start) {
            ...
            if (time === 0) time = 30;
            start = true;   //记录下已经开始这个状态
            if (gameover) {     //如果是从上局的游戏结束开始的，那么清除上一局的积分
                Score.value = 0;
                score = 0;
                gameover = false;
            } else {
            ...
            reset();
            enable();       //使得游戏开始，鼠标点击有用
            create_mole();  //随机生成地鼠位置函数
            settime();      //计时函数
            //alert(start);
        } else {
            //alert("stop");
            document.getElementById('status').value = "Stop";
            start = false;  //记录下暂停这个状态
            stopCount();    //停止计时
            disabled();     //暂停状态下不可以进行游戏
        }
    };
    ...
    function create_mole () {
        num = Math.random();//得到一个0到1之间的随机数
        num = Math.ceil(num * 60);//num*60的取值范围在0~60之间,使用向上取整就可以得到一个1~60的随机数
