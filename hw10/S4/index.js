...
(function () {
        ...
        $(".mask").click(function (event) {
            ...
            $.get("mask.txt",function (data) {             //请求的URL,只要不是服务器规定的几种形式，服务器端都会返回随机数
                if ($(".mask .number").hasClass('unread') && $(".mask .number").text() == "...") {
                    ...
                    r.checkIfCanSum();      //检查是否可以求和了
                    if (r.count < 5) {          //如果还有按钮没点击，则按照随机结果的顺序点击下一个按钮
                        r.randomClick(r.randomArray[r.count++]);
                    }
                }
            },"text");
    ...
    r.ListenapbClick = function () {
        $('.apb').click(function(){
            ...
            r.randomArray = buttonArray.sort(randomSort);       //生成随机数组
            $('#order').text("顺序:"+r.randomArray);
            r.count = 0;
            $('.apb').unbind("click");          //在计算器重置之前@+只能点击一次
            r.randomClick(r.randomArray[r.count++]);        //将随机数组作为参数传递下去，按顺序模拟点击
        }.bind(this));
    };
})();